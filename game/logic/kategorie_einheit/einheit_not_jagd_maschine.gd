extends RefCounted
class_name Einheit_NotJagdMaschine
## Not-Jagd der Verzweiflung: Sie waehlt den schwaechsten Nachbarn in
## Reichweite, wenn weder ein jagdbares Tier in Reichweite noch Fleisch im
## Lager ist. Der teure Tier- und Nachbarschafts-Scan traegt seinen eigenen
## Performance-Cache, der genau einen Takt lebt. Die Ausfuehrung der Tat
## bleibt in der Ernte-Maschine; diese Maschine waehlt nur das Ziel.

## Kategorie daten: Quellen und der Reichweiten-Cache.
...
## Kategorie logik: Opfer waehlen, Reichweite cachen.
...
func cache_erneuern(nummer: int) -> void:
	...
	## dies einmal pro Tick auf, bevor die verstreuten Pruefungen starten.
	if _tier_cache_tick != nummer:
		_tier_cache_tick = nummer
		_tier_reichweite_cache.clear()

func opfer_fuer(jaeger_index: int) -> int:
	## Schwaechster Nachbar in Reichweite: Reichweite und Mindest-HP stehen
