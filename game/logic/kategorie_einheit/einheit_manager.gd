...
## und die Verdrahtung; Lesefragen trägt die Basis, Regeln die Maschinen.
...
func _enter_tree() -> void:
	# Die Weltuhr wird zur Laufzeit aufgeloest, damit Headless-Laeufe laufen.
...
func need_baum_setzen(baum: Pop_NeedBaum) -> void:
	# Der Need-Tree besitzt die Mood-Maschinen als Kinder.
	_verdrahtung.need_baum_erneuern(self, baum)

func sozial_lese_ruf_setzen(ruf: Callable) -> void:
	# Jede neue Einheit bekommt ihre Gerücht-Blase.
...
func weg_planung_aktualisieren() -> void:
	# Nach einer Gebäudeplatzierung kennt das Netz das Hindernis.
...
func verteilung_wert() -> float:
	# Der Regler-Lesewert ist der geltende Verbrauch.
...
func moral_grundsatz_lesen(schalter_id: String) -> bool:
	return _verdrahtung.moral_grundsatz(schalter_id)

func moral_grundsatz_setzen(schalter_id: String, erlaubt: bool) -> void:
	_verdrahtung.moral_grundsatz_aendern(self, schalter_id, erlaubt)

func job_vergeben(einheit_index: int, job_id: String, ziel_typ: Job_Basis.ZielTyp, ziel_index: int, ziel_position: Vector2) -> bool:
	# Blickrichtung und Darsteller-Folge trägt die Verdrahtung.
...
func schlag_empfaenger_setzen(empfaenger: Callable) -> void:
	# Die Progressions-Domäne hört jeden Arbeitsschlag.
	_verdrahtung.schlag_empfaenger_setzen(empfaenger)

func _auf_tick(nummer: int, delta: float) -> void:
	# Vor dem Tick wandert der Lager-Snapshot zur Mood-Kette.
...
func _auf_zustand_geaendert(neu: int, status: Einheit_Status, mood: Pop_MoodMaschine) -> void:
	# Trupp merkt den Zustand, der Job-Fluss die Stimmungs-Regel.
