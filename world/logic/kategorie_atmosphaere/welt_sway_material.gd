...
## Orts-Versatz kommen deterministisch aus der Welt-Position (MODEL_MATRIX[3].x).
...
void vertex() {
	...
	// Der Phasenversatz wird aus der Welt-Koordinate (MODEL_MATRIX[3].x)
	// abgeleitet, sodass Baeume desselben Typs dieselbe Material-Instanz
	// teilen koennen und Draw-Call-Batching erhalten bleibt.
	float gewicht = VERTEX.y / max(1.0, 160.0);
	float wind = wind_minimum + wind_spanne * (0.5 + 0.5 * sin(TIME * 6.2831853 * wind_frequenz * 24.0));
	float phase_versatz = MODEL_MATRIX[3].x * 0.02;
...
## Materialien werden pro Parameter-Kombination gecacht, damit alle gleichen
## Objekte dieselbe ShaderMaterial-Instanz teilen (Draw-Call-Batching).
func material_fuer(objekt: Objekt_Basis, _lokal_x: float = 0.0) -> ShaderMaterial:
	...
	var schwingung := float(objekt.schluessel_daten.get("wind_sway_schwingung", 0.008))
	var schluessel := "%s_%.4f_%.4f" % [objekt.element_id, wucht * richtung_zeichen, schwingung]
	if _material_cache.has(schluessel):
		return _material_cache[schluessel] as ShaderMaterial
	var material := ShaderMaterial.new()
	material.shader = shader_instanz()
	material.set_shader_parameter("staerke", wucht * richtung_zeichen)
	material.set_shader_parameter("schwingung", schwingung)
	...
	_material_cache[schluessel] = material
	return material
