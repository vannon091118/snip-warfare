...
const SCHWELLE_OZEAN_HEIGHT := 0.15
const SCHWELLE_GEBIRGE_HEIGHT := 0.6
const SCHWELLE_TUNDRA_TEMPERATUR := 0.3
const SCHWELLE_STEPPE_FEUCHT := 0.3
...
func analysiere_biom(height: float, moisture: float, temperature: float) -> String:
	...
	var h_norm := (height + 1.0) * 0.5
	var m_norm := (moisture + 1.0) * 0.5
	var t_norm := (temperature + 1.0) * 0.5
	...
	var steppe_conf := _schwellenwerte.get("steppe", {})
...
func find_biom_eintrag(biom_id: String) -> Dictionary:
	# Lade biome.json einmal und suche den Eintrag
	var biome_pfad := "res://world/data/biome.json"
	var biome_daten: Dictionary = {}
	try:
		var datei := open(biome_pfad)
		biome_daten = parse_json(datei.get_as_text())
		datei.close()
	catch err:
		push_warning("Biome-JSON konnte nicht geladen werden: %s" % [err])
		return {}
