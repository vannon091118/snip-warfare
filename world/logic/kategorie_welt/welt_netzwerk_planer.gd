...
func _wege_berechnen(model: Welt_Model) -> void:
	...
	for f: Welt_Fraktion in _fraktionen:
		f.nachbarn.clear()
	# Der Name spieler_region schattet die Methode spieler_region() – deshalb lokaler Alias.
	var aktive_spieler_region: Vector2i = _spieler_region
	var spieler_pos_kachel := aktive_spieler_region * model.region_kante + Vector2i(int(model.region_kante * 0.5), int(model.region_kante * 0.5))
	...
	for f in _fraktionen:
		if not _linie_frei(model, aktive_spieler_region, _region_von_fraktion(model, f)):
			continue
