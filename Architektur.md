...
Diese Datei ist die verbindliche Pflichtdokumentation der Projektarchitektur. Sie wird aktiv gepflegt: Jede Änderung an Klassennamen, Kategorien, Datenfeldern oder Domänengrenzen wird hier sofort mitgepflegt. Vor jedem Commit wird `python tools/preflight.py` ausgeführt; nur ein fehlerfreier Preflight (Exit-Code 0) gilt als abnahmefähig. Maschinenlesbarer Router ist die Index-Familie: [`INDEX.md`](INDEX.md) als Wurzel mit Index-Familie und Klasseninventar, [`INDEX_DOMAENEN.md`](INDEX_DOMAENEN.md) mit der Signal- und Array-Matrix je Domäne, [`INDEX_DATEN.md`](INDEX_DATEN.md) mit jedem JSON-Pool samt Besitzer und Verbrauchern und die eine Last-Datei [`INDEX_LETZTE_AENDERUNG.md`](INDEX_LETZTE_AENDERUNG.md) mit dem Delta des letzten Index-Laufs; erzeugt von `python tools/index_generieren.py`.
...
Neue Datensätze entstehen nur über Datenpool → Registry → Maschine. `Ui_EinheitPanel` liest Einheiten nur über `Einheit_Manager.einheit_beschreibung()` / `einheit_position()`, `Ui_TierPanel` nur über `Tier_Manager.tier_bestand()` und deckelt die Anzeige auf `ANZEIGE_MAX`. Kein Panel greift auf `_einheiten` oder `_tiere`. Fenster sind PackedScenes, die Szene stempelt per `PackedScene.instantiate()` + `add_child()` unter den HUD-CanvasLayer, setzt danach `einrichten()`; `_ready()` der Szene läuft garantiert, `get_node_or_null()` schützt jeden Zugriff.

Einheit- und Tierfenster liegen zusammen in `Ui_DebugPanelSzene` (`ui/scenes/hud/hud_debug_panel.gd`), das im Normalbetrieb unsichtbar ist und nur über den Debug-Schalter des Eingabe-Übersetzers (F3, `debug_umgeschaltet`) sichtbar wird. Die Spielszene hält nur diesen einen Schalter; kein Frame schreibt Panel-Sichtbarkeit nachträglich um.

`Welt_TerrainBlatt` (`world/logic/kategorie_welt/welt_terrain_blatt.gd`) ist die einzige Stelle, die aus der Kachel-Datenklasse und der Kachelkoordinate die sichtbare Variante ableitet (Spiegelung und Tönung). Sie zieht ihre Zahl aus `Kern_Zufall.abgeleitet_fuer`, hält keine Textur und keine Szene und wird ausschließlich vom `Welt_Renderer` gelesen.

Der Objektschatten ist ein Blob-System: `Welt_ObjektKnoten.schatten_wurf_setzen` legt je Objekt eine weiche Ellipse (`GradientTexture2D`, radial, geteilte statische Textur) an den Fußpunkt und skaliert sie exakt mit der Objektgröße aus dem Katalog (`schatten_wurf` in `element_katalog.json`). Größere Objekte werfen größere Schatten, kleine kaum einen; es gibt keine `LightOccluder2D` mehr und `Welt_PapierLicht` wirft bewusst keine Okkluder-Schatten (`shadow_enabled = false`), sonst läge ein grauer Schleier über der ganzen Karte. Das Licht bleibt reines Stimmungslicht mit Energie und Farbe je Tagesphase.

`Ui_FensterLeiste` (`ui/scenes/hud/fenster_leiste.gd`) sitzt unten am Bildrand (`PRESET_BOTTOM_WIDE`, 70 px) und baut ihre Knöpfe garantiert im `_ready`, falls `einrichten()` vor dem Baumeintritt nur die Einträge vorab lagert. Die Spielszene meldet ihr die Fenster-Einträge (Bau, Karte, Debug, Grundsätze, Menü) als reine Daten-Arrays mit Callables; die Leiste kennt keine Fenster, nur die Callables. Das Grundsatz-Fenster (`Ui_GrundsatzFenster`) öffnet über seinen Eintrag und liest die Schalter über `Ui_GrundsatzPanel` und die statische Brücke `Pop_MoralInstanz.schalter_ids_fuer()` ohne eigene Moral-Logik.
...
| `game/data/steuerung.json` | Menschenlesbare Steuerung WASD Kamera, Linksklick einzeln, Drag Masse, Rechtsklick Kontext sammeln abbauen marschieren mit Tooltip Werkzeug, faktor 1=10s; je Aktion die Sperre `gesperrt_ab_stufe` als einzige Stufen-Wahrheit und die `ziel_tags`, die bestimmen, worauf die Aktion angeboten wird | `Kern_SteuerungRegistry` -> `Ui_EingabeSteuerung` (Kamera, Auswahl, Kontext, `_ziel_tags_fuer_ort`) + `Ui_KameraSteuerung` + `KontextMenue` (Tag-Abgleich und ausgegraute Sperren) + `Kern_SteuerungRegistry.inputmap_registrieren` (einzige InputMap-Schreibstelle: erzeugt aus `kamera.tasten` und `kamera.alternativ_tasten` die Aktionen `kamera_hoch/links/runter/rechts` inklusive Pfeil-Gruppe, gelesen über `Input.get_vector` in `Ui_KameraSteuerung` und `karten_editor.gd`) |
...
| `world/data/element_katalog.json` | Platzierbare Objekte mit Kategorie; Terrain-Kacheln (typ `kachel`) tragen `varianten`, `spiegelbar` und `toenungen`, alle anklickbaren Objekte tragen `ziel_tags` | `Welt_Registry` (`finde_objekt`, `ziel_tags_fuer`) -> `Welt_Renderer` + `Welt_TerrainBlatt` + `KontextMenue` |
| `world/data/tier_verhalten.json` | Tierwerte (Trigger, Geschwindigkeit, Ertrag, HP) | `Tier_Verhalten` |
| `world/data/biome.json` | Biome als Mutationen je Biom mit logik_id, modifikator, faktor; `barriere` markiert unpassierbare Landschaften (Gebirge, Ozean) | `Welt_BiomRegistry` + `Welt_BiomMutation` + `Welt_BiomManager`; `barriere` lesen `Welt_NetzwerkPlaner` und `welt_map.gd` |
...
| `world/data/weltkarte_definition.json` | Makrokarte der Startauswahl: Regionszahl 16 mal 12, Regionkante in Kacheln, Startbiom | `Welt_MakroGenerator` (plant nur Regionen) -> `Welt_NetzwerkPlaner` -> `welt_map.gd` |
| `world/data/generator_gewichte.json` (biome) | Biomgewichte mit `ebene` (`lokal` oder `makro`): Gebirge und Ozean sind Makro-Landschaften, die lokale Karte zieht sie nie | `Welt_GeneratorVerteilung.ziehe_eintrag_mit(..., ebene)` -> lokaler Generator (`lokal`) und `Welt_MakroGenerator` (alle Ebenen) |
...
| `population/data/bindung.json` | Bindungsstufen (Fremd bis Gefaehrtin) mit Wirkung, Schutz und Verfall sowie die Ereignisse, die eine Bindung wachsen oder sinken lassen; jede Zahl ist eine Datenentscheidung, kein Code kennt sie hart | Daten sind angeliefert; Leser wird die Bindungs-Schnittstelle aus Slice 14 (CP-14.2, CP-14.4) |
| `world/data/moral_regeln.json` | Kolonie-Grundsaetze (Kannibalismus, Tierbevorzugung, Schutz der Bindungsobjekte, Verhungern), Ersatzhandlungen bei Blockade und Haltung je Rasse; Schwellen bleiben ausschliesslich in mood_modifikatoren.json | Daten sind angeliefert; Leser wird die Zielwahl der `Einheit_VerhaltensMaschine` aus Slice 14 (CP-14.3, CP-14.4) |
...
5. **Gebäude und Produktion:** Datenbesitzer sind `gebaeude.json` und `generator_gewichte.json` (Cluster-Regeln). Erzeugung: `Gebaeude_DefinitionRegistry` als einzige Erweiterungsgrenze, Bau- und Produktionsmaschinen sind generisch. Die Belegungsregel `belegt_kachel` je Gebäude und die einzige Bau-Buchungsstelle `Gebaeude_Manager.bauen_anfordern` (mit `_bauplatz_frei`) verhindern Doppelbauten auf derselben Kachel; die Statuszeile des HUD kommt als Signal `status_geaendert` und wird nur bei echter Änderung gemeldet. Erweiterungsgrenze: neues Gebäude nur als Katalog-, Gewichte- und Gebäude-Eintrag plus Asset; eine neue Produktionskette ist ein Rezept-Eintrag, kein Code-Sonderfall.
...
7. **World- und Karten-Ebene:** Datenbesitzer sind `welt_definition.json`, `weltkarte_definition.json` und `biome.json`. Erzeugung: `Welt_Ladevorgang` ist die einzige Laden-und-Erzeugen-Stelle der lokalen Karte, `Welt_MakroGenerator` die einzige Stelle der Makrokarte (sie plant nur Regionen und ruft nie den lokalen Generator), `Welt_MapFabrik` die einzige Karten-Erzeugung, `Welt_Speicher` die einzige Persistenz. Besitz: genau eine Kartenwahrheit, die World zeigt auf dieselbe `Welt_Model`-Instanz wie die Szene (`model_uebernehmen`), `Ui_WeltSitzung` trägt nur den Verweis. Barrieren sind eine Eigenschaft des Bioms; Fraktionsplatzierung, Startbereich und Wegenetz meidet sie im `Welt_NetzwerkPlaner`. Erweiterungsgrenze: neue Karte nur über die Fabrik, neue Makro-Landschaft nur als Biom plus Gewichte-Eintrag mit `ebene`, nie eine zweite Erzeugungs- oder Speicherstelle.
8. **UI:** Datenbesitzer sind `steuerung.json` (Kamera, Auswahl, Kontext-Aktionen mit logik_id, Sperrstufe und ziel_tags) und `element_katalog.json` (ziel_tags je Objekt). Erzeugung: `Ui_EingabeSteuerung` und `Ui_KameraSteuerung` übersetzen Eingabe in Maschinen-Aufrufe; Panels sind gepackte Szenen, die nur lesen, das Debug-Fenster `Ui_DebugPanelSzene` bündelt die zwei Beobachter. Erweiterungsgrenze: neue Kontext-Aktion nur als Eintrag mit logik_id, ziel_tags und einem Decode-Zweig; wer ein neues Ziel anklickbar machen will, vergibt am Objekt einen Tag und schreibt keinen UI-Filter; die UI erfindet keinen Zustand und rechnet keine Fachwerte.
...
1. Neue Klasse: Präfix vergeben, in den Kategorie-Ordner legen, ggf. Datenklassen-Erzeugung registrieren, Architektur.md in Abschnitt 1 und 3 ergänzen, danach `python tools/index_generieren.py` laufen lassen, damit die ganze Index-Familie den neuen Stand erzählt.
...
7. Versionierung: Die Datei `VERSION` ist die einzige Versionsquelle und startete mit `V0.01`; jeder Bump erhöht um `0.01`. `python tools/version_bump.py` hebt die Version an und zieht `README.md`, `ROADMAP.md`, `INDEX.md`, diese Datei und `AGENTS.md` im selben Lauf nach. Die Prüfkategorie `version` (E043) vergleicht jedes Dokument mit `VERSION` und blockiert jeden Commit, dessen Dokumentation hinterherhinkt. `python tools/version_bump.py --pruefen` meldet den Ist-Stand ohne Änderung. Der Bump zieht zusätzlich die Index-Familie nach.
8. Index-Familie: Der Code ist die Wahrheit, die Indizes folgen. `python tools/index_generieren.py` erzeugt vier Dateien aus dem Code: `INDEX.md` (Wurzel mit Index-Familie und Klasseninventar), `INDEX_DOMAENEN.md` (Klassen, Signal-Matrix mit den Rollen D Deklaration, S Senden, V Verbinden, und Array-Matrix der `Array[Typ]` je Domäne), `INDEX_DATEN.md` (jeder JSON-Pool mit Besitzer, obersten Schlüsseln und namentlichen Verbrauchern) und genau eine Last-Datei `INDEX_LETZTE_AENDERUNG.md`, die den Delta des letzten Index-Laufs bildlich mitschreibt. Die Bausteine liegen granular in `tools/index/` (kern, inventar, matrix, daten, root_index, domaenen_index, daten_index, letzte_aenderung, erzeugen). Die Prüfkategorie `index` (E044) rechnet den Stand beim Preflight neu und meldet jede Abweichung mit Datei und Zeile; wer eine Index-Datei von Hand ändert oder nach einem Umbau nicht neu erzeugt, scheitert am Preflight.
