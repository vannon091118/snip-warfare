...
1. **Kern:** Datenbesitzer sind `kern_modifikatoren.json` und `modifikator_settings.json` über `Kern_ModifikatorRegistry.geteilte()` (genau eine Ladung). Erzeugung: Maschinen-Instanzen werden von ihren besitzenden State-Maschinen gehalten, die Registry-Daten fließen über `Kern_ModifikatorMaschine`. Erweiterungsgrenze: neuer Modifikator nur als Eintrag in `kern_modifikatoren.json`, neue Rechenbereiche nur als Bereich-Eintrag in `modifikator_settings.json`, nie eine zweite Formel. Die `Kern_LogikRegistry` bleibt markierte Plugin-Grenze ohne Verbraucher; ihr Anschluss geschieht nur über einen eigenen Auftrag mit sichtbarer Verifikation.
2. **Welt-Objekte und Tiere:** Datenbesitzer sind `element_katalog.json` und `tier_verhalten.json`. Erzeugung: `Welt_Registry._objekt_klasse_fuer` (geschlossene Zuordnung der neun Objektklassen, Fallback `Objekt_Basis`) und `Tier_Registry._tier_klasse_fuer` (script-Feld-Fallback); Spawn nur über Generator-Gewichte (`generator_gewichte.json` Cluster mit Biome-Liste und Chance) beziehungsweise `Welt_TierPlatzierer` (typ `bewegt`). Erweiterungsgrenze: neues Objekt über Katalog-Eintrag plus Asset (geprüft durch `Kern_AssetPruefer`) plus Klasse in der Zuordnung; neues Tier über Katalog, Verhaltenswerte, script-Feld und Cluster-Eintrag, ohne Änderung an Status, Manager oder Darsteller.
3. **Jobs und Einheiten:** Datenbesitzer sind `job_config.json` (script-Feld für alle sechs Jobs) und `ressourcen.json`. Erzeugung: `Job_Registry._job_erzeugen` lädt das script mit Existenz- und Typprüfung; `Einheit_Manager` besitzt die Einheiten und tickt sie. Erweiterungsgrenze: neuer Job nur als Klasse plus script-Eintrag im Pool; neue Ressource nur als Pool-Eintrag plus Klasse in `_ressourcen_klasse_fuer` mit Fallback.
